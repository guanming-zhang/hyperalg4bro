...
cdef class _Cdef_InversePower(_Cdef_BasePotential):

    def __cinit__(self, double a, double eps, np.ndarray radii, np.ndarray boxv, 
        ...
        if method == Distance.CARTESIAN:
            ...
            if enable_batch:
                if self.ndim == 1:
                    self.baseptr = shared_ptr[cppBasePotential](<cppBasePotential*> new \
                        cppInversePowerPeriodicBatchCellLists[INT1](self.a, self.eps, self.radii, self.boxv, self.ncellx_scale, self.balance_omp))
                elif self.ndim == 2:
                    self.baseptr = shared_ptr[cppBasePotential](<cppBasePotential*> new \
                        cppInversePowerPeriodicBatchCellLists[INT2](self.a, self.eps, self.radii, self.boxv, self.ncellx_scale, self.balance_omp))
                elif self.ndim == 3:
                    self.baseptr = shared_ptr[cppBasePotential](<cppBasePotential*> new \
                        cppInversePowerPeriodicBatchCellLists[INT3](self.a, self.eps, self.radii, self.boxv, self.ncellx_scale, self.balance_omp))
                elif self.ndim == 4:
                    self.baseptr = shared_ptr[cppBasePotential](<cppBasePotential*> new \
                        cppInversePowerPeriodicBatchCellLists[INT4](self.a, self.eps, self.radii, self.boxv, self.ncellx_scale, self.balance_omp))
                else:
                    raise NotImplementedError
            elif use_cell_lists:
                if self.ndim == 1:
