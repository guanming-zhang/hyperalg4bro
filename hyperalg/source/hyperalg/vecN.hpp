#ifndef HYPERALG_ENT_VECN_H
#define HYPERALG_ENT_VECN_H
...
ha::VecN<N, dtype> dot(MatrixNM<N, M, dtype> const & A, ha::VecN<M, dtype> const & v)
{
    ha::VecN<N, dtype> C(0);
    for (size_t i = 0; i<N; ++i){
        double val = 0;
...
template<size_t N, size_t M, typename dtype>
ha::MatrixNM<M, N, dtype> transpose(MatrixNM<N, M, dtype> const & A)
{
    ha::MatrixNM<M, N, dtype> mat;
    for (size_t i = 0; i<N; ++i){
        for (size_t k = 0; k<M; ++k){
            mat(k,i) = A(i,k);
        }
...
template<size_t N, typename dtype=double>
ha::MatrixNM<N, N, dtype> identity()
{
    ha::MatrixNM<N, N, dtype> A(0.);
    for (size_t i = 0; i<N; ++i) {
        A(i,i) = 1.;
    }
    return A;
