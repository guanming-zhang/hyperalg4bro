...
protected:
    ...
    // properties for noise in gradient calculations(noise is in the unit of radium_sum)
    double m_mean,m_sigma;
    int m_noise_type; //0--nonreciprocal uniform, 1--reciprocal uniform 
                      ...
                      // nonreciprocal: the noisy force on a pair of particles is oppsite and not necessarily of the same magnitude
    std::mt19937 m_gen;
    std::normal_distribution<double> m_ndis;
    std::uniform_real_distribution<double> m_udis;

    virtual double random_number(){
        if (m_noise_type < 0){
            return 0.0;
        }
        else if (m_sigma == 0.0 || abs(m_sigma) < 1e-15){
                return m_mean;
        }
        else if (m_noise_type <= 1){
            return m_udis(m_gen);
        }
        else if (m_noise_type >= 2){
            return m_ndis(m_gen);
        }
    }
...
public:
    ...
    virtual void set_gradient_noise(const double mean,const double sigma, int noise_type){
        m_mean = mean;
        m_sigma = sigma;
        m_noise_type = noise_type;
        if (noise_type <= 1){
            // for unifrom distribution in (a,b) sigma = (b-a)/(sqrt(12))
            double a = mean - sqrt(3.0)*sigma, b= mean + sqrt(3.0)*sigma;
            m_udis.reset();
            m_udis.param(std::uniform_real_distribution<double>::param_type(a, b));
        }
        else if(noise_type >= 2){
            m_ndis.reset();
            m_ndis.param(std::normal_distribution<double>::param_type(mean, sigma));
        }
    }
    void insert_atom_pair(const size_t atom_i, const size_t atom_j, const size_t isubdom)
    {
        ...
        *m_energies[0] += m_interaction->energy_gradient(r2, &gij, radius_sum);
        #endif
        if ((m_noise_type >= 0) && (sqrt(r2)<radius_sum)){
            if ((m_noise_type == 0) || (m_noise_type == 2)){
                for (size_t k = 0; k < m_ndim; ++k) {
                    double dr_hat_k = dr[k]/sqrt(r2);
                    (*m_gradient)[xi_off + k] -= dr_hat_k*random_number();
                    (*m_gradient)[xj_off + k] += dr_hat_k*random_number();
                }
            }
            else if ((m_noise_type == 1) || (m_noise_type == 3)){
                double rnd = random_number();
                for (size_t k = 0; k < m_ndim; ++k) {
                    double dr_hat_k = dr[k]/sqrt(r2);
                    (*m_gradient)[xi_off + k] -= dr_hat_k*rnd;
                    (*m_gradient)[xj_off + k] += dr_hat_k*rnd;
                }
            }
        }
        if (gij != 0) {
...
public:
    ...
    virtual void set_gradient_noise(const double mean,const double sigma, int noise_type){
        m_egAcc.set_gradient_noise(mean,sigma,noise_type);
    }
